...
struct ArraySplitImpl {
    static Status execute(VExprContext* context, doris::vectorized::Block* block,
                          int* result_column_id, const DataTypePtr& result_type,
                          const std::vector<VExpr*>& children, bool reverse) {
...
class ArraySplitFunction : public LambdaFunction {
    ENABLE_FACTORY_CREATOR(ArraySplitFunction);

public:
    ~ArraySplitFunction() override = default;

    static constexpr auto name = "array_split";
    ...
    doris::Status execute(VExprContext* context, doris::vectorized::Block* block,
        ...
        ArraySplitImpl::execute(context, block, result_column_id, result_type, children, false);

        return Status::OK();
    }
};

class ArrayReverseSplitFunction : public LambdaFunction {
    ENABLE_FACTORY_CREATOR(ArrayReverseSplitFunction);

public:
    ~ArrayReverseSplitFunction() override = default;

    static constexpr auto name = "array_reverse_split";

    static LambdaFunctionPtr create() { return std::make_shared<ArrayReverseSplitFunction>(); }
...
void register_function_array_split(doris::vectorized::LambdaFunctionFactory& factory) {
    factory.register_function<ArraySplitFunction>();
}

void register_function_array_reverse_split(doris::vectorized::LambdaFunctionFactory& factory) {
    factory.register_function<ArrayReverseSplitFunction>();
}
} // namespace doris::vectorized